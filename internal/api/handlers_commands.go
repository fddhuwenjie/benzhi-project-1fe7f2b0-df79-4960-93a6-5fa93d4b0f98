...
func (s *Server) HandleRegisterItem(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.service.RegisterItem(r.PathValue("batch_id"), meta(r, req.RequestID, req.ExpectedRevision), application.RegisterItem{ItemID: req.ItemID, ShelfMark: req.ShelfMark, PaperType: req.PaperType, BaselineSurfacePH: req.BaselineSurfacePH, BaselineColdExtractPH: req.BaselineColdExtractPH, MeasurementPoints: req.MeasurementPoints, SourceDigest: req.SourceDigest})
	s.finish(w, result, err)
}
...
func (s *Server) HandleTreatmentRound(w http.ResponseWriter, r *http.Request) {
	...
	result, err := s.service.SubmitTreatmentRoundContext(r.Context(), r.PathValue("batch_id"), meta(r, req.RequestID, req.ExpectedRevision), application.SubmitRound{RoundID: req.RoundID, RoundKind: req.RoundKind, StartedAt: req.StartedAt.Time, CompletedAt: req.CompletedAt.Time, Measurements: req.Measurements, EvidenceDigest: req.EvidenceDigest})
	s.finish(w, result, err)
}
