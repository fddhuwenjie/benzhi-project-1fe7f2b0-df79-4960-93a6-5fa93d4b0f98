...
func Open(root string) (*Repository, error) {
	...
	r := &Repository{root: root, cache: make(map[string]cachedSnapshot)}
...
func (r *Repository) loadUnlocked(batchID string) (Snapshot, error) {
	...
	digest := domain.Digest(raw)
	snap, ok := r.cached(batchID, digest)
	if !ok {
		if err := json.Unmarshal(raw, &snap); err != nil {
			return Snapshot{}, domain.Errorf(domain.CodeEvidenceCorrupt, "快照 JSON 损坏")
		}
		r.remember(batchID, digest, snap)
	}
...
func (r *Repository) remember(batchID, digest string, snap Snapshot) {
	r.cacheMu.Lock()
