...
func (r *Repository) Load(batchID string) (Snapshot, error) {
	return r.LoadContext(context.Background(), batchID)
}

func (r *Repository) LoadContext(ctx context.Context, batchID string) (Snapshot, error) {
	...
	lock.RLock()
	defer lock.RUnlock()
	return r.loadUnlockedContext(ctx, batchID)
}

func (r *Repository) loadUnlocked(batchID string) (Snapshot, error) {
	return r.loadUnlockedContext(context.Background(), batchID)
}

func (r *Repository) loadUnlockedContext(ctx context.Context, batchID string) (Snapshot, error) {
	if err := ctx.Err(); err != nil {
		return Snapshot{}, err
	}
