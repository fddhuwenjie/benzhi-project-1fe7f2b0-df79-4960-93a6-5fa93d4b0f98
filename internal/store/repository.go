...
func (r *Repository) loadUnlocked(batchID string) (Snapshot, error) {
	snap, err := r.loadSnapshotUnlocked(batchID)
	if err != nil {
		return Snapshot{}, err
	}
...
func (r *Repository) loadSnapshotUnlocked(batchID string) (Snapshot, error) {
	raw, err := os.ReadFile(r.snapshotPath(batchID))
	if errors.Is(err, fs.ErrNotExist) {
...
func (r *Repository) QueryTimeline(batchID string, query TimelineQuery) (TimelinePage, error) {
	return r.QueryTimelineContext(context.Background(), batchID, query)
}

func (r *Repository) QueryTimelineContext(ctx context.Context, batchID string, query TimelineQuery) (TimelinePage, error) {
