...
func (s *Service) QueryTimeline(batchID string, query TimelineQuery) (store.TimelinePage, error) {
	return s.QueryTimelineContext(context.Background(), batchID, query)
}
func (s *Service) QueryTimelineContext(ctx context.Context, batchID string, query TimelineQuery) (store.TimelinePage, error) {
	return s.repo.QueryTimelineContext(ctx, batchID, store.TimelineQuery{Cursor: query.Cursor, Limit: query.Limit, EventType: query.EventType, ActorID: query.ActorID, MinRevision: query.MinRevision, MaxRevision: query.MaxRevision, SnapshotAnchor: query.SnapshotAnchor})
}
