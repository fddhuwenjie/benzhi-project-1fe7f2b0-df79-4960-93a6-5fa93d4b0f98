...
func newPreflightCache(limit int) *preflightCache {
	return &preflightCache{entries: make([]preflightCacheEntry, 0, limit)}
}
...
func (c *preflightCache) get(key preflightCacheKey) (RoundPreflightReport, bool) {
	...
	return RoundPreflightReport{}, false
}
...
func NewService(repo *store.Repository) *Service {
	return &Service{repo: repo, coord: NewCoordinator(128), now: time.Now, preflightCache: newPreflightCache(64)}
}
func NewServiceWithClock(repo *store.Repository, clock Clock) *Service {
	return &Service{repo: repo, coord: NewCoordinator(128), now: clock, preflightCache: newPreflightCache(64)}
}
...
func (s *Service) PreflightTreatmentRound(batchID string, meta CommandMeta, in SubmitRound) (RoundPreflightReport, error) {
	...
	inputDigest, _, err := evidence.Digest(in)
	if err != nil {
		return RoundPreflightReport{}, err
	}
	unlock := s.coord.lock(batchID)
	...
	cacheKey := preflightCacheKey{batchID: batchID, revision: snap.Batch.Revision, inputDigest: inputDigest}
	if cached, ok := s.preflightCache.get(cacheKey); ok {
		return cached, nil
	}
	round := domain.TreatmentRound{RoundID: in.RoundID, RoundKind: in.RoundKind, SubmittedBy: meta.ActorID, StartedAt: in.StartedAt, CompletedAt: in.CompletedAt, Measurements: append([]domain.Measurement(nil), in.Measurements...), EvidenceDigest: in.EvidenceDigest}
	...
	report := RoundPreflightReport{CurrentRevision: snap.Batch.Revision, Baseline: BaselineSummary{Digest: snap.Batch.BaselineDigest, ItemCount: len(snap.Batch.Items), Standards: snap.Batch.Standards}, Preview: preview}
	s.preflightCache.put(cacheKey, report)
	return report, nil
}

func cloneRoundPreflightReport(report RoundPreflightReport) RoundPreflightReport {
	copy := report
	copy.Preview.Items = append([]domain.RoundItemResult(nil), report.Preview.Items...)
	for i := range copy.Preview.Items {
		copy.Preview.Items[i].FailureCodes = append([]string(nil), report.Preview.Items[i].FailureCodes...)
	}
	return copy
}
